...
public class VentanaEliminarTratamiento extends JFrame implements WindowListener, ActionListener
{
	...
	JLabel mensaje1 = new JLabel("¿Estás seguro que quieres eliminar este Tratamiento?");
	...
	public VentanaEliminarTratamiento()
	{
		...
		try //USAMOS UN WHILE PARA RELLENAR EL JCOMBOX CON LOS RESULTADOS DEL RESULSET
		{
			...
			{
				choice1.addItem(rs_id_pacientes.getString("idPaciente") +"-"+ rs_nombres_pacientes.getString("nombrePaciente") +" "+ rs_apellidos_pacientes.getString("apellidosPaciente"));//AÑADE NOMBRE Y APELLIDOS DE LOS RS USADOS ANTERIORMENTE Y LOS METE EN UN JCOMBOBOX
			}
		...
		if(a.equals(btnB1))
		{
			...
			if(!nombre_completo_paciente.equals("Elegir Paciente..."))
			{
				...
				try //USAMOS UN WHILE PARA RELLENAR EL JCOMBOX CON LOS RESULTADOS DEL RESULSET
				{
					...
					{
						choice2.addItem(rs_tratamientos.getString(1) + "-IMPORTE: " + rs_tratamientos.getString(4) +"€  FECHA: "+ rs_tratamientos.getString(5));//AÑADE NOMBRE Y APELLIDOS DE LOS RS USADOS ANTERIORMENTE Y LOS METE EN UN JCOMBOBOX
					}
				} catch (SQLException e)
				{
					e.printStackTrace();
